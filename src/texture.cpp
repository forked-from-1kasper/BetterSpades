...
int texture_create(struct texture* t, char* filename) {
    ...
    glGenTextures(1, (GLuint*) &t->texture_id);
    glBindTexture(GL_TEXTURE_2D, t->texture_id);
...
int texture_create_buffer(struct texture* t, int width, int height, unsigned char* buff, int is_new) {
    if(is_new) glGenTextures(1, (GLuint*) &t->texture_id);
    t->width = width;
    t->height = height;
    t->pixels = buff;
    texture_resize_pow2(t, maxc(width, height));
...
void texture_delete(struct texture* t) {
    if(t->pixels)
        free(t->pixels);
    glDeleteTextures(1, (const GLuint*) &t->texture_id);
}
...
void texture_resize_pow2(struct texture* t, int min_size) {
    ...
    max_size = maxc(max_size, min_size);

    int w = 1, h = 1;
    if(strstr((char*) glGetString(GL_EXTENSIONS), "ARB_texture_non_power_of_two") != NULL) {
    ...
    auto pixels_new = (unsigned int*) malloc(w * h * sizeof(unsigned int));
    ...
    for(int y = 0; y < h; y++) {
        for(int x = 0; x < w; x++) {
            ...
            unsigned int ba = ((unsigned int*)t->pixels)[minc((int)px + (int)py * t->width + 1, t->width * t->height)];
            unsigned int ab
                = ((unsigned int*)t->pixels)[minc((int)px + (int)py * t->width + t->width, t->width * t->height)];
            unsigned int bb
                = ((unsigned int*)t->pixels)[minc((int)px + (int)py * t->width + 1 + t->width, t->width * t->height)];
            pixels_new[x + y * w] = 0;
...
unsigned int texture_block_color(int x, int y) {
    ...
    if(x < 4) {
        ...
        return rgb(
            minc(base[0][y] + x * (base[0][y] * 2 + 2) * (base[0][y] > 0) + ((x - 3) * 64 - 33) * (!base[0][y]), 255),
            minc(base[1][y] + x * (base[1][y] * 2 + 2) * (base[1][y] > 0) + ((x - 3) * 64 - 33) * (!base[1][y]), 255),
            minc(base[2][y] + x * (base[2][y] * 2 + 2) * (base[2][y] > 0) + ((x - 3) * 64 - 33) * (!base[2][y]), 255));
    }
}
...
void texture_gradient_fog(unsigned int* gradient) {
    ...
    for(int y = 0; y < size; y++) {
        for(int x = 0; x < size; x++) {
            int d = minc(sqrt(distance2D(size / 2, size / 2, x, y)) / (float)size * 2.0F * 255.0F, 255);
...
void texture_init() {
    ...
    auto gradient = (unsigned int*) malloc(512 * 512 * sizeof(unsigned int));
    ...
    unsigned char vect[] {0, 0, 0, 0};
    texture_create_buffer(&texture_dummy, 1, 1, (unsigned char*) &vect, 1);
}
