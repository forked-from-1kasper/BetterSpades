                   ...
                   && (window_time() - players[local_player_id].item_showup) >= 0.5F && local_player_blocks > 0) {
                    ...
                    if(pos != NULL && pos[1] > 1
                       ...
                       && !(pos[0] == (int)camera_x && pos[1] == (int)camera_y - 1 && pos[2] == (int)camera_z)) {
                        ...
                        local_player_blocks = maxc(local_player_blocks - 1, 0);
            ...
            while(mu_next_command(ctx, &cmd)) {
                switch(cmd->type) {
                    ...
                    case MU_COMMAND_ICON: {
                        ...
                        int size = minc(cmd->icon.rect.w, cmd->icon.rect.h);
...
void text_input(struct window_instance* window, unsigned int codepoint) {
    char text[] {codepoint, 0};
    if (hud_active->ctx)
        mu_input_text(hud_active->ctx, (const char*) &text);
...
void keys(struct window_instance* window, int key, int scancode, int action, int mods) {
    ...
    if(key == WINDOW_KEY_FULLSCREEN && action == WINDOW_PRESS) { // switch between fullscreen
        ...
        if(!settings.fullscreen) {
            glfwSetWindowMonitor((GLFWwindow*) window->impl, glfwGetPrimaryMonitor(), 0, 0, mode->width, mode->height,
                                 mode->refreshRate);
            settings.fullscreen = 1;
        } else {
            glfwSetWindowMonitor((GLFWwindow*) window->impl, NULL, (mode->width - 800) / 2, (mode->height - 600) / 2, 800, 600, 0);
    ...
    if(key == WINDOW_KEY_SCREENSHOT && action == WINDOW_PRESS) { // take screenshot
        ...
        auto pic_data = (unsigned char*) malloc(settings.window_width * settings.window_height * 4 * 2);
