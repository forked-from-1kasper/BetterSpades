...
void hud_init() {
    hud_serverlist.ctx = (mu_Context*) malloc(sizeof(mu_Context));
    hud_settings.ctx   = (mu_Context*) malloc(sizeof(mu_Context));
    hud_controls.ctx   = (mu_Context*) malloc(sizeof(mu_Context));
...
void hud_change(struct hud* hud) {
...
static void hud_ingame_render(mu_Context* ctx, float scalex, float scalef) {
    ...
    if(window_key_down(WINDOW_KEY_NETWORKSTATS)) {
        ...
        int max = 0;
        for(int k = 0; k < 40; k++) {
            max = maxc(max, network_stats[k].ingoing + network_stats[k].outgoing);
        }
        for(int k = 0; k < 40; k++) {
            ...
            float ping_h = minc(network_stats[39 - k].avg_ping / 25.0F, 160.0F);
    ...
    if(network_map_transfer) {
        ...
        if(gamestate.gamemode_type == GAMEMODE_TC && gamestate.progressbar.tent < gamestate.gamemode.tc.territory_count
           && gamestate.gamemode.tc.territory[gamestate.progressbar.tent].team
            ...
            float p = maxc(minc(gamestate.progressbar.progress
        ...
        if (camera_mode == CameraMode::FPS && window_time() - players[local_player_id].item_showup >= 0.5F) {
            if (players[local_player_id].held_item == TOOL_GRENADE && local_player_grenades > 0) {
                if (action == WINDOW_RELEASE) {
                    ...
                    g.fuse_length
                        = maxc(3.0F - (window_time() - players[local_player_id].input.buttons.lmb_start), 0.0F);
    ...
    if(chat_input_mode == CHAT_NO_INPUT) {
        ...
        if(action == WINDOW_PRESS) {
            ...
            if(key == WINDOW_KEY_VOLUME_UP) {
                settings.volume = minc(settings.volume + 1, 10);
            }
            if(key == WINDOW_KEY_VOLUME_DOWN) {
                settings.volume = maxc(settings.volume - 1, 0);
            }
    ...
    if(screen_current == SCREEN_NONE) {
        ...
        if((camera_mode == CameraMode::FPS || camera_mode == CameraMode::SPECTATOR)
           && distance2D(f->start.x, f->start.y, settings.window_height * 0.3F, settings.window_height * 0.7F)
            ...
            float my = maxc(minc(y - settings.window_height * 0.7F, settings.window_height * 0.2F),
                           -settings.window_height * 0.2F);
...
static void hud_serverlist_pingupdate(void* e, float time_delta, char* aos) {
    ...
    if(!e) {
        ...
        serverlist = (serverlist_entry*) realloc(serverlist, (++server_count) * sizeof(struct serverlist_entry));
...
static void server_c(char* address, char* name) {
    if (file_exists(address)) {
        auto data = file_load(address);
        map_vxl_load((uintptr_t) data, file_size(address));
        free(data);
        chunk_rebuild_all();
        camera_mode = CameraMode::FPS;
...
static void hud_serverlist_render(mu_Context* ctx, float scalex, float scaley) {
    ...
    if(mu_begin_window_ex(ctx, "Main", frame, MU_OPT_NOFRAME | MU_OPT_NOTITLE | MU_OPT_NORESIZE)) {
        ...
        int row1[] {A, B, C, -1};
        mu_layout_row(ctx, 4, (int*) &row1, 0);
        ...
        mu_layout_row(ctx, 1, (int*) &empty_row, -1);
        ...
        int row3[]{0.12F * width, 0.418F * width, 0.22F * width, 0.117F * width, -1};
        mu_layout_row(ctx, 5, (int*) &row3, 0);
        ...
        int row4[] {0.12F * width, flag_width, 0.418F * width - flag_width - ctx->style->spacing * 2,
                    0.22F * width, 0.117F * width, -1};
        mu_layout_row(ctx, 6, (int*) &row4, 0);
        ...
        if(server_count > 0) {
            ...
            mu_layout_row(ctx, 1, (int*) &empty_row, 0);
    ...
    if(window_time() - chat_popup_timer < chat_popup_duration
       && mu_begin_window_ex(ctx, "Disconnected from server", mu_rect(200, 250, 300, 100),
        ...
        int row6[] {ctx->text_width(ctx->style->font, "Reason:", 0) * 1.5F, -1};
        mu_layout_row(ctx, 2, (int*) &row6, 0);
        mu_text(ctx, "Reason:");
    ...
    if(request_news) {
        switch(http_process(request_news)) {
            case HTTP_STATUS_COMPLETED: {
                JSON_Value* js = json_parse_string((const char*) request_news->response_data);
                ...
                for(int k = 0; k < news_entries; k++) {
                    ...
                    if(json_object_get_string(s, "image")) {
                        ...
                        int width, height;
                        lodepng_decode32(&buffer, (unsigned int*) &width, (unsigned int*) &height, (const unsigned char*) img, size);
                        texture_create_buffer(&current->image, width, height, buffer, 1);
                        texture_filter(&current->image, TEXTURE_FILTER_LINEAR);
                    }
                    current->next = (k < news_entries - 1) ? (serverlist_news_entry*) malloc(sizeof(struct serverlist_news_entry)) : NULL;
                    current = current->next;
                }
    ...
    if(request_version) {
        switch(http_process(request_version)) {
            case HTTP_STATUS_COMPLETED:
                ...
                serverlist_is_outdated = strcmp((const char*) request_version->response_data, BETTERSPADES_VERSION) != 0;
    ...
    if(request_serverlist) {
        switch(http_process(request_serverlist)) {
            ...
            case HTTP_STATUS_COMPLETED: {
                JSON_Value* js = json_parse_string((const char*) request_serverlist->response_data);
...
static void hud_settings_render(mu_Context* ctx, float scalex, float scaley) {
    ...
    if(mu_begin_window_ex(ctx, "Main", frame, MU_OPT_NOFRAME | MU_OPT_NOTITLE | MU_OPT_NORESIZE)) {
        ...
        int row1[] {A, B, C, -1}; mu_layout_row(ctx, 4, (int*) &row1, 0);
        ...
        if(mu_header_ex(ctx, "All settings", MU_OPT_EXPANDED)) {
            ...
            int row4[] {0.65F * width, -0.05F * width, -1};
            mu_layout_row(ctx, 3, (int*) &row4, 0);
            mu_layout_next(ctx);
            ...
            if (*a.display) {
                ...
                if (open) {
                    ...
                    if (a.def != a.original) {
                        ...
                        int row3[] {0.65F * width, -0.05F * width, -1};
