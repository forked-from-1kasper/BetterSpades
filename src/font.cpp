...
void font_init() {
    font_vertex_buffer = (short int*) malloc(512 * 8 * sizeof(short));
    CHECK_ALLOCATION_ERROR(font_vertex_buffer)
    font_coords_buffer = (short int*) malloc(512 * 8 * sizeof(short));
...
static font_backed_data* font_find(float h) {
    if (font_current_type == FONT_SMALLFNT) h *= 1.5F;

    struct font_backed_id id {
        .type = font_current_type,
        .size = h,
    };

    auto f_cached = (font_backed_data*) ht_lookup(&fonts_backed, &id);
    if (f_cached) return f_cached;
    ...
    f.w = 64; f.h = 64;
    f.cdata = (stbtt_bakedchar*) malloc((0xFF - FONT_BAKE_START) * sizeof(stbtt_bakedchar));
    ...
    while(1) {
        ...
        res = stbtt_BakeFontBitmap(
            (const unsigned char*) file, 0, h, (unsigned char*) temp_bitmap,
            f.w, f.h, FONT_BAKE_START, 0xFF - FONT_BAKE_START, f.cdata
        );
        if (res > 0 || (f.w == max_size && f.h == max_size)) break;
