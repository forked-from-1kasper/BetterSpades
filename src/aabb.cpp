...
bool aabb_intersection_terrain(AABB* a, int miny) {
    ...
    int min_x = minc(maxc(floor(a->min_x) - 1, 0), map_size_x);
    int min_y = minc(maxc(floor(a->min_y) - 1, miny), map_size_y);
    int min_z = minc(maxc(floor(a->min_z) - 1, 0), map_size_z);

    int max_x = minc(maxc(ceil(a->max_x) + 1, 0), map_size_x);
    int max_y = minc(maxc(ceil(a->max_y) + 1, 0), map_size_y);
    int max_z = minc(maxc(ceil(a->max_z) + 1, 0), map_size_z);
