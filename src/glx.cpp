...
void glx_enable_sphericalfog() {
    ...
    if (!settings.smooth_fog) {
        ...
        float params1[] {1.0F / settings.render_distance / 2.0F, 0.0F, 0.0F,
                              -camera_x / settings.render_distance / 2.0F + 0.5F};
        float params2[] {0.0F, 0.0F, 1.0F / settings.render_distance / 2.0F,
                              -camera_z / settings.render_distance / 2.0F + 0.5F};
        glTexGenfv(GL_T, GL_EYE_PLANE, (float*) &params1);
        glTexGenfv(GL_S, GL_EYE_PLANE, (float*) &params2);
        ...
        glLightModelfv(GL_LIGHT_MODEL_AMBIENT, (float*) &color);

        float coord[] {camera_x, (settings.render_distance * map_size_y) / 16.0F, camera_z, 1.0F};
        glLightfv(GL_LIGHT1, GL_POSITION, (float*) &coord);

        float spot[] {0.0F, -1.0F, 0.0F}; float diffuse[] {1.0F, 1.0F, 1.0F, 1.0F};
        glLightfv(GL_LIGHT1, GL_SPOT_DIRECTION, (float*) &spot);
        glLightfv(GL_LIGHT1, GL_DIFFUSE, (float*) &diffuse);

        float ambient[] {-fog_color[0], -fog_color[1], -fog_color[2], 1.0F};
        glLightfv(GL_LIGHT1, GL_AMBIENT, (float*) &ambient);
